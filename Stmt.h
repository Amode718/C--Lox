...
struct Stmt {virtual std::any accept(StmtVisitor& visitor) = 0;
};
...
struct Switch: Stmt, public std::enable_shared_from_this<Switch> {
  Switch(std::shared_ptr<Expr> condition, std::vector<std::shared_ptr<Case>> cases, std::shared_ptr<Stmt> defaultCase)
  ...
  const std::shared_ptr<Stmt> defaultCase;
};
